...
---Attempts to build at the current position using build lock
---@param pindex number
---@param build_state fa.BuildLock.BuildState
---@param current_position MapPosition
---@param movement_direction defines.direction
---@return string action BuildAction constant
local function attempt_build(pindex, build_state, current_position, movement_direction)
   local player = game.get_player(pindex)
   if not player then return BuildAction.RETRY end
   ...
   -- Let backend handle the entire build process
   logger:debug(
      string.format(
         "Backend: %s, pos=(%.1f,%.1f), move_dir=%d, build_dir=%d",
         backend.name,
         current_position.x,
         current_position.y,
         movement_direction,
   ...
   -- Create helpers instance with context
