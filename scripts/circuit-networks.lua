...
function mod.localise_signal_name(signal, pindex)
   ...
   elseif sig_type == nil or sig_type == "item" then
      -- When sig_type is nil, it defaults to "item" type
...
function mod.constant_combinator_signals_info(ent, pindex)
   ...
   if valid_signals_count == 0 then
      ...
      table.insert(result, " with signals ")
      ...
      for i = 1, max_signals_count, 1 do
         ...
         if signal.signal ~= nil then
            ...
            if signal_index > 1 then table.insert(result, " and ") end
            table.insert(result, signal_name)
            table.insert(result, " times ")
            table.insert(result, tostring(signal.count))
            table.insert(result, ", ")
         end
      end
   end
...
function mod.constant_combinator_add_stack_signal(ent, stack, pindex)
   ...
   Speech.speak(pindex, { "", "Added signal for ", localising.get_localised_name_with_fallback(stack.prototype) })
end
...
function mod.constant_combinator_add_selector_signal(prototype, signal_type, ent, pindex)
   ...
   Speech.speak(pindex, { "", "Added signal for ", localising.get_localised_name_with_fallback(prototype) })
end
...
function mod.constant_combinator_remove_last_signal(ent, pindex)
   ...
   for i = max_signals_count, 1, -1 do
      ...
      if signal.signal ~= nil then
         local signal_name = mod.localise_signal_name(signal.signal, pindex)
         Speech.speak(pindex, { "", "Removed last signal ", signal_name })
         combinator.set_signal(i, nil)
         return
      end
   end
   Speech.speak(pindex, { "fa.circuit-no-signals-to-remove" })
end
...
function mod.read_circuit_condition(pindex, ent, comparator_in_words)
   ...
   if cond.second_signal == nil then
      ...
      if type(second_signal_name) == "table" then
         table.insert(result, second_signal_name)
      else
         table.insert(result, tostring(second_signal_name))
      end
   end
...
function mod.circuit_network_menu_run(pindex, ent_in, menu_index, clicked, other_input)
   ...
   if nwr == nil and nwg == nil then
      nw_name = " none "
      -- Speech.speak(pindex, { "fa.circuit-no-network-connected" })
      -- return
   ...
   elseif index == 1 then
      ...
      if not clicked then
         ...
         Speech.speak(pindex, { "fa.circuit-list-active-signals" })
      else
         if nwr == nil and nwg == nil then
            Speech.speak(pindex, { "fa.circuit-no-network-connected" })
            return
   ...
   elseif index == 2 then
      ...
      if not clicked then
         Speech.speak(pindex, { "fa.circuit-list-members" })
      else
         if nwr == nil and nwg == nil then
            Speech.speak(pindex, { "fa.circuit-no-network-connected" })
            return
   ...
   elseif ent.type == "constant-combinator" then
      ...
      if index == 4 then
         ...
         if not clicked then
            Speech.speak(pindex, { "fa.circuit-read-combinator-signals" })
         else
      ...
      elseif index == 5 then
         ...
         if not clicked then
            Speech.speak(pindex, { "fa.circuit-add-signal-from-selector" })
         else
            Speech.speak(
               pindex,
               "Signal selection is temporarily unavailable while the selector system is being redesigned."
            )
         end
      elseif index == 6 then
         --Set a new count for the last signal
         if not clicked then
            Speech.speak(pindex, { "fa.circuit-set-new-count" })
         else
      ...
      elseif index == 7 then
         ...
         if not clicked then
            Speech.speak(pindex, { "fa.circuit-delete-last-signal" })
         else
      ...
      elseif index == 8 then
         --Toggle switch
         if not clicked then
            Speech.speak(pindex, { "fa.circuit-toggle-switch" })
         else
            ent.get_control_behavior().enabled = not ent.get_control_behavior().enabled
            local enabled = ent.get_control_behavior().enabled
            if enabled == true then
               Speech.speak(pindex, { "fa.circuit-switched-on" })
            elseif enabled == false then
               Speech.speak(pindex, { "fa.circuit-switched-off" })
            end
         end
      elseif index > 8 then
         --(inventory edge: play sound and set index and call this menu again)
      ...
      local control = ent.get_control_behavior()
      if control == nil then
         Speech.speak(pindex, { "fa.circuit-no-interface" })
         return
      end
      if nwr == nil and nwg == nil then
         Speech.speak(pindex, { "fa.circuit-no-network-connected" })
      ...
      if index == 4 then
         ...
         if not clicked then
            Speech.speak(pindex, { "fa.circuit-read-machine-behavior" })
         else
      ...
      elseif index == 5 then
         ...
         if not clicked then
            Speech.speak(pindex, { "fa.circuit-toggle-reading-mode", read_mode })
         else
      ...
      elseif index == 6 then
         ...
         if not clicked then
            Speech.speak(pindex, { "fa.circuit-toggle-operation-mode", op_mode })
         else
      ...
      elseif index == 7 then
         ...
         if not clicked then
            Speech.speak(pindex, { "fa.circuit-toggle-enabled-condition" })
         else
      ...
      elseif index == 8 then
         ...
         if not clicked then
            Speech.speak(pindex, { "fa.circuit-set-first-signal" })
         else
            local result = "Not using a condition"
            if uses_condition == true then
               result =
                  "Setting circuit conditions is temporarily unavailable while the signal selector is being redesigned."
            end
            Speech.speak(pindex, result)
            p.play_sound({ path = "Inventory-Move" })
         end
      elseif index == 9 then
         ...
         if not clicked then
            Speech.speak(pindex, { "fa.circuit-set-second-signal" })
         else
            local result = "Not using a condition"
            if uses_condition == true then
               result =
      ...
      elseif index == 10 then
         ...
         if not clicked then
            Speech.speak(pindex, { "fa.circuit-set-constant-number" })
         else
            local result = "Not using a condition"
         ...
         if ent.type ~= "programmable-speaker" or (ent.type == "programmable-speaker" and control == nil) then
            if index == 11 then
               Speech.speak(pindex, { "fa.circuit-speaker-menu-note" })
            ...
            if index == 11 then
               ...
               if not clicked then
                  Speech.speak(pindex, { "fa.circuit-toggle-playback" })
               else
                  local params = ent.parameters
                  params.playback_globally = not params.playback_globally
                  ent.parameters = params
                  if ent.parameters.playback_globally then
                     Speech.speak(pindex, { "fa.circuit-playback-global" })
                  else
                     Speech.speak(pindex, { "fa.circuit-playback-local" })
                  end
            ...
            elseif index == 14 then
               ...
               if not clicked then
                  Speech.speak(pindex, { "fa.circuit-switch-instrument-up" })
               else
                  --Switch up
                  local params = control.circuit_parameters
                  ...
                  control.circuit_parameters = params
                  --Read instrument
                  Speech.speak(pindex, instruments[ins_id].name)
                  mod.play_selected_speaker_note(ent)
               end
            elseif index == 15 then
               --Switch instrument down
               if not clicked then
                  Speech.speak(pindex, { "fa.circuit-switch-instrument-down" })
               else
                  --Switch down
                  local params = control.circuit_parameters
                  local ins_id = params.instrument_id
            ...
            elseif index == 16 then
               --Switch note up
               if not clicked then
                  Speech.speak(pindex, { "fa.circuit-switch-note-up" })
               else
                  local params = control.circuit_parameters
                  local ins_id = params.instrument_id
                  local note_id = params.note_id
                  ...
                  control.circuit_parameters = params
                  --Read note
                  Speech.speak(pindex, notes[note_id])
                  mod.play_selected_speaker_note(ent)
               end
            elseif index == 17 then
               --Switch note down
               if not clicked then
                  Speech.speak(pindex, { "fa.circuit-switch-note-down" })
               else
                  local params = control.circuit_parameters
                  local ins_id = params.instrument_id
                  local note_id = params.note_id
            ...
            elseif index == 18 then
               ...
               if not clicked then
                  Speech.speak(pindex, { "fa.circuit-toggle-note-pitch" })
               else
                  local params = control.circuit_parameters
                  params.signal_value_is_pitch = not params.signal_value_is_pitch
                  control.circuit_parameters = params
                  if control.circuit_parameters.signal_value_is_pitch then
                     Speech.speak(pindex, { "fa.circuit-enabled" })
                  else
                     Speech.speak(pindex, { "fa.circuit-disabled" })
                  end
                  mod.play_selected_speaker_note(ent)
               end
            elseif index == 19 then
               ...
               if not clicked then
                  Speech.speak(pindex, { "fa.circuit-toggle-polyphony" })
               else
                  local params = ent.parameters
                  params.allow_polyphony = not params.allow_polyphony
                  ent.parameters = params
                  if ent.parameters.allow_polyphony then
                     Speech.speak(pindex, { "fa.circuit-enabled-polyphony" })
                  else
                     Speech.speak(pindex, { "fa.circuit-disabled-polyphony" })
                  end
                  mod.play_selected_speaker_note(ent)
               end
...
function mod.circuit_network_neighbors_info(pindex, ent, wire_type)
   ...
   for i, member in ipairs(members_list) do
      if member.unit_number ~= ent.unit_number then
         ...
         table.insert(result, tostring(math.ceil(util.distance(member.position, ent.position))))
         table.insert(result, " ")
         table.insert(result, FaUtils.direction_lookup(FaUtils.get_direction_biased(member.position, ent.position)))
         table.insert(result, ", ")
      end
   end
   return result
...
function mod.circuit_network_members_info(pindex, ent, wire_type)
   ...
   if ent_counter > 0 then table.insert(result, " and ") end
   table.insert(result, tostring(pole_counter))
   table.insert(result, " electric poles, ")
   return result
end
...
function mod.circuit_network_signals_info(pindex, nw, color_name, group_no)
   ...
   for i, sig in ipairs(signals) do
      ...
      if sig_type == "item" then
         local proto = prototypes.item[sig_name]
         sig_local_name = proto and localising.get_localised_name_with_fallback(proto) or sig_name
      elseif sig_type == "fluid" then
      ...
      if i >= first_sig and i <= last_sig then
         table.insert(result, sig_local_name)
         table.insert(result, " times ")
         table.insert(result, sig_count)
         table.insert(result, ", ")
