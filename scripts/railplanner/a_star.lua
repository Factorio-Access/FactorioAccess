...
local function expand_geometry_rotate(geo)
	...
	if geo.two_way_rotational_symmetry then
		end_rotation = dirs16.east
	end
...
local function try_to_add(data,node,edge)
   ...
   if next_node.elevation < 0 then
      return nil
   end
   if next_node.elevation > max_elevated_height then
      return nil
   end
   --todo: check collisions

   if data.to[1] == next_node.pos[1] and data.to[2] == next_node.pos[2] then
      if not data.to_dir or data.to_dir== next_node.dir then
         return next_node
      end
   end
   add_node(data,node)
