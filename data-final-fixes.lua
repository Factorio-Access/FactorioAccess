...
--[[
Make pipes walkable by characters while still blocking enemies.

The problem: characters and enemies share the same collision layers (is_object, player, train).
If we just remove "player" from pipes, enemies would also walk through.

The solution:
1. Find all entities with is_object but not player, add player to them
2. Remove is_object from characters (so characters only collide via "player" layer)
3. Remove player from pipes

Result:
- Pipes have is_object but not player
- Characters have player but not is_object
- Enemies have both is_object and player
- Therefore: enemies collide with pipes (via is_object), characters don't
]]

local collision_mask_util = require("collision-mask-util")

-- Step 1: Find ALL prototypes with is_object, add player if missing
-- This handles mods, Space Age, and future additions
local prototypes_with_is_object = collision_mask_util.collect_prototypes_with_layer("is_object")
for _, proto in pairs(prototypes_with_is_object) do
   local mask = collision_mask_util.get_mask(proto)
   if not mask.layers["player"] then
      mask.layers["player"] = true
      proto.collision_mask = mask
   end
end

-- Step 2: Remove is_object from all characters
for name, proto in pairs(data.raw["character"] or {}) do
   local mask = collision_mask_util.get_mask(proto)
   mask.layers["is_object"] = nil
   proto.collision_mask = mask
end

-- Step 3: Remove player from all pipes
for name, proto in pairs(data.raw["pipe"] or {}) do
   local mask = collision_mask_util.get_mask(proto)
   mask.layers["player"] = nil
   proto.collision_mask = mask
end
